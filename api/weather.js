export default async function handler(req, res) {
    ...
    try {
        ...
        let currentUrl;
        if (lat && lon) {
            currentUrl = `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=${API_KEY}&units=${unitsParam}`;
        } else {
            currentUrl = `https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${API_KEY}&units=${unitsParam}`;
        }
        ...
        const [forecastRes, airRes, geoRes] = await Promise.all([
            fetch(`https://api.openweathermap.org/data/2.5/forecast?lat=${fetchedLat}&lon=${fetchedLon}&appid=${API_KEY}&units=${unitsParam}`),
